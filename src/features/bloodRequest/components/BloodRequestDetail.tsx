import React from 'react';
import { BloodRequest, RequestStatus, UrgencyLevel } from '../types';
import { format } from 'date-fns';

interface BloodRequestDetailProps {
  request: BloodRequest;
  onUpdateStatus?: (status: RequestStatus) => void;
  onDelete?: () => void;
  onMatch?: () => void;
  isLoading?: boolean;
  showActions?: boolean;
}

const BloodRequestDetail: React.FC<BloodRequestDetailProps> = ({
  request,
  onUpdateStatus,
  onDelete,
  onMatch,
  isLoading = false,
  showActions = true
}) => {
  // Format blood type for display
  const formatBloodType = (type: string) => {
    return type.replace('_', '+').replace('NEGATIVE', '-').replace('POSITIVE', '+');
  };

  // Get urgency level label and color
  const getUrgencyInfo = (level: UrgencyLevel) => {
    switch (level) {
      case UrgencyLevel.LOW:
        return { label: 'Low', color: 'bg-blue-100 text-blue-800' };
      case UrgencyLevel.NORMAL:
        return { label: 'Normal', color: 'bg-green-100 text-green-800' };
      case UrgencyLevel.HIGH:
        return { label: 'High', color: 'bg-yellow-100 text-yellow-800' };
      case UrgencyLevel.CRITICAL:
        return { label: 'Critical', color: 'bg-red-100 text-red-800' };
      default:
        return { label: 'Unknown', color: 'bg-gray-100 text-gray-800' };
    }
  };

  // Get status label and color
  const getStatusInfo = (status: RequestStatus) => {
    switch (status) {
      case RequestStatus.PENDING:
        return { label: 'Pending', color: 'bg-yellow-100 text-yellow-800' };
      case RequestStatus.MATCHING:
        return { label: 'Matching', color: 'bg-blue-100 text-blue-800' };
      case RequestStatus.FULFILLED:
        return { label: 'Fulfilled', color: 'bg-green-100 text-green-800' };
      case RequestStatus.CANCELLED:
        return { label: 'Cancelled', color: 'bg-gray-100 text-gray-800' };
      default:
        return { label: 'Unknown', color: 'bg-gray-100 text-gray-800' };
    }
  };

  const urgencyInfo = getUrgencyInfo(request.urgency_level);
  const statusInfo = getStatusInfo(request.status);
  
  // Format dates
  const formatDate = (dateString: string) => {
    try {
      return format(new Date(dateString), 'MMM dd, yyyy');
    } catch (error) {
      return 'Invalid date';
    }
  };

  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
        <div>
          <h3 className="text-lg leading-6 font-medium text-gray-900">
            Blood Request Details
          </h3>
          <p className="mt-1 max-w-2xl text-sm text-gray-500">
            Created on {formatDate(request.created_at)}
          </p>
        </div>
        <div className="flex space-x-2">
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${urgencyInfo.color}`}>
            {urgencyInfo.label}
          </span>
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusInfo.color}`}>
            {statusInfo.label}
          </span>
        </div>
      </div>
      <div className="border-t border-gray-200">
        <dl>
          <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Patient Name</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{request.patient_name}</dd>
          </div>
          <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Blood Type</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{formatBloodType(request.blood_type)}</dd>
          </div>
          <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Units Needed</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{request.units_needed}</dd>
          </div>
          <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Required By</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{formatDate(request.required_by_date)}</dd>
          </div>
          <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Hospital</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
              {request.hospital_name}<br />
              {request.hospital_address}<br />
              {request.hospital_city}, {request.hospital_state} {request.hospital_postal_code}
            </dd>
          </div>
          {request.medical_notes && (
            <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500">Medical Notes</dt>
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{request.medical_notes}</dd>
            </div>
          )}
          <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Contact Information</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
              Phone: {request.contact_phone}<br />
              Email: {request.contact_email}
            </dd>
          </div>
        </dl>
      </div>
      
      {showActions && (
        <div className="px-4 py-3 bg-gray-50 text-right sm:px-6 space-x-2">
          {request.status === RequestStatus.PENDING && onMatch && (
            <button
              type="button"
              onClick={onMatch}
              disabled={isLoading}
              className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-300"
            >
              Find Donors
            </button>
          )}
          
          {onUpdateStatus && request.status !== RequestStatus.FULFILLED && request.status !== RequestStatus.CANCELLED && (
            <button
              type="button"
              onClick={() => onUpdateStatus(RequestStatus.FULFILLED)}
              disabled={isLoading}
              className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:bg-green-300"
            >
              Mark as Fulfilled
            </button>
          )}
          
          {onUpdateStatus && request.status !== RequestStatus.CANCELLED && (
            <button
              type="button"
              onClick={() => onUpdateStatus(RequestStatus.CANCELLED)}
              disabled={isLoading}
              className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled:bg-gray-300"
            >
              Cancel Request
            </button>
          )}
          
          {onDelete && (
            <button
              type="button"
              onClick={onDelete}
              disabled={isLoading}
              className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:bg-red-300"
            >
              Delete
            </button>
          )}
        </div>
      )}
    </div>
  );
};

export default BloodRequestDetail; 